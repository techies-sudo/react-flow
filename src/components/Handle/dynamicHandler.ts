...
export function onMouseDown(
  ...
  var elementBelowIsSource = elementBelow?.classList.contains("source");
  const elementBelowIsUpdater = elementBelow?.classList.contains(
    "react-flow__edgeupdater"
  );
  //onMousedown
  if (elementBelowIsTarget) {
    elementBelow?.classList.replace("target", "source");
    elementBelowIsTarget = false;
    elementBelowIsSource = true;
  }

  if (
    !reactFlowNode ||
    (!elementBelowIsTarget && !elementBelowIsSource && !elementBelowIsUpdater)
  ) {
    return;
  }
  ...
  console.log("before onMouseMove");
  ...
  doc.addEventListener(
    "mousemove",
    // onMouseMove as EventListenerOrEventListenerObject
    () => {console.log("onmousemove added");}
  );
  doc.addEventListener(
    "mouseup",
    // onMouseUp as EventListenerOrEventListenerObject
    () => {console.log("onmouseup added");}
  );
}
